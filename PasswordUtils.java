...
public class PasswordUtils {
  ...
  public static byte[] hashPassword(String password) {
    ...
    try {
      ...
      e.printStackTrace();
      throw new IllegalStateException("Issue generating hashPassword");
      //return new byte[144];
    }
  }
  ...
  public static boolean plaintextMatchesHash(String plaintext, byte[] saltedHashed) {
    // TODO: extract the salt from the byte array (ie, undo the logic you implemented in getHash),
    // then use it to check whether the user-provided plaintext password matches the password hash.
    //byte[] salt = saltedHashed[0, SALT_LENGTH];
    byte[] salt = Arrays.copyOfRange(saltedHashed, 0, SALT_LENGTH);
    //byte[] hash = saltedHashed[SALT_LENGTH, saltedHashed.length];
    byte[] hash = Arrays.copyOfRange(saltedHashed, SALT_LENGTH, saltedHashed.length);

    byte[] newHash = generateSaltedPassword(plaintext, salt);
